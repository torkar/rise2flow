...
3. Daily surveys
3. An exit survey, at time $t_{0+11}$ (MAAS, etc. and IPIP)
...
After joining these two datasets we have `r length(table(d$ID))` subjects in the dataset and the total sample size is $n=$ `r nrow(d)`.
...
1. Weekly trends modeled with Gaussian Processes (results in Sect. \@ref(weekly)).
2. Daily trends modeled with Gaussian Processes (results in Sect. \@ref(daily)). We've also shown conditional effects of `gender` since it is the main driver for deviations.
...
Each of these models has $n=$ `r nrow(m_maas$data)`.
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
mcmc_areas(m_maas, prob = 0.5, prob_outer = 0.95, regex_pars = "Gender01$")
```
...
TODO: Will do the same as above with all plots below (2021-02-26)
