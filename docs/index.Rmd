...
We want to model the weekly instrument and how it varies over time $t$. In this case, $t$ will be modeled using a Gaussian Process and then each individual in the dataset will be modeled using a varying intercept. Varying intercepts of `ID` should be modeled as correlated (since we actuallt have five parts we model, i.e., five questions). We can assume that the answers should correlate.
...
Looks good. Now sample with data.
...
Gender has a negative effect (male/transmale responses are lower), however it is actually *not* significant. The only thing we can say is that a number of parameters makes respondents, on average, respond with lower values at $t_1$.
