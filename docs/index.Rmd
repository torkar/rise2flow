...
For the models we want to predict the outcome given LiveSessions and RecordedSessions as main effects. Additionally we will use a varying intercept for each $ID$.
...
p <- get_prior(formula_maas, data=d2, family=cumulative(probit)) %>%
  ...
  mutate(prior = ifelse(class == "Intercept", "normal(0,5)", prior)) %>%
  mutate(prior = ifelse(class == "sd", "weibull(2,1)", prior)) %>%
  mutate(prior = ifelse(class == "simo", "dirichlet(2)", prior)
         )
