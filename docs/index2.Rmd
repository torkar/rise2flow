...
During spring 2021 the Rise 2 Flow study was replicated in yet another experiment `E2`. The setup and execution was very similar to the first instance (`E1`) of the experiment. In the `data/` directory one can find data for both instances `E1/` and `E2/`. The purpose of this document is to show how the analysis of the experiments was done. With this replication package anyone can download and rerun the analysis and testing our assumptions.
...
```{r load, echo=TRUE}
entryExit <- read.xlsx(here("data/E2/Quant Comp data entry.xlsx")) # all instruments
weekly <- read.xlsx(here("data/E2/compiled.xlsx"), detectDates = T) 
pers <- read.xlsx(here("data/E2/Metadata.xlsx")) # personal data
daily <- read.xlsx(here("data/E2/R2F2 Daily.xlsx"), detectDates = T)
```
...
pers$Gender_1_2 <- ifelse(pers$`What.is.your.gender?` == "Man/Transman", 1, 2)
...
pers$Occupation_1_2 <- ifelse(pers$`What.is.your.gender?` == "Student", 1, 2)

# make living condition a categorical factor (unordered)
pers$Living_1_4 <- factor(pers$`What.is.your.living.situation?`, levels = c("I live by myself", "I live in shared housing", "I live with a partner", "I live with my family"))

# cleanup
pers$`How.old.are.you?`[c(29,30)] = "21" # "21 years old"

# convert to numeric
pers$Age <- as.numeric(pers$`How.old.are.you?`)

# remove the columns now
pers[,2:6] <- NULL
```
...
#### `weekly` model (WM): Prior and posterior predictive checks
...
#### WM: Parameter estimates {#trendWeekly}
...
#### `daily` model (DM): Prior and posterior predictive checks
...
#### DM: Parameter estimates {#trendDaily}
...
### Multivariate models (MM) with temporal variable
...
#### MM: Prior and posterior predictive checks
